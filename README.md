...
<body>
    ...
    <script>
        ...
        // Configuración de límites y tasas
        const loanConfig = {
            active: {
                maxAmount: 30000,
                maxTerm: 12,
                interestRate: 0.015 // 1.5% mensual
            },
            inactive: {
                union: {
                    maxAmount: 6000,
                    maxTerm: 6,
                    interestRate: 0.03 // 3% mensual
                },
                nonunion: {
                    maxAmount: 10000,
                    maxTerm: 6,
        ...
        // Manejar cambio en el estado de membresía
        memberStatusSelect.addEventListener('change', function() {
            const status = this.value;
            
            if (status === 'inactive') {
                unionStatusDiv.classList.remove('hidden');
                unionMemberSelect.required = true;
            } else {
                unionStatusDiv.classList.add('hidden');
        ...
        // Manejar cambio en el estado sindical
        unionMemberSelect.addEventListener('change', function() {
            updateTermOptions();
            validateAmount();
        });

        // Actualizar opciones de plazo según el tipo de usuario
        ...
        // Validar monto según el tipo de usuario
        function validateAmount() {
            ...
            if (status === 'active') {
                maxAmount = loanConfig.active.maxAmount;
            } else if (status === 'inactive') {
                if (unionStatus === 'union') {
                    maxAmount = loanConfig.inactive.union.maxAmount;
                } else if (unionStatus === 'nonunion') {
                    maxAmount = loanConfig.inactive.nonunion.maxAmount;
                }
            }
        ...
        // Validar monto al cambiar el input
        ...
        form.addEventListener('submit', function(e) {
            ...
            if (status === 'active') {
                interestRate = loanConfig.active.interestRate;
            } else if (status === 'inactive') {
                if (unionStatus === 'union') {
                    interestRate = loanConfig.inactive.union.interestRate;
                } else if (unionStatus === 'nonunion') {
                    interestRate = loanConfig.inactive.nonunion.interestRate;
                }
            }
            ...
            let weeklyPayment = 0;
            if (status === 'active' && term === 12) {
                // Para miembros activos con 12 meses, dividir entre 52 semanas
                weeklyPayment = totalAmount / 52;
            } else {
                // Para otros casos, dividir entre (meses * 4) semanas
                weeklyPayment = totalAmount / (term * 4);
            }
            
            // Mostrar resultados, advertencia y botón de impresión
